...
router.get('/', async function (req, res, next) {
    ...
    if (user) {
        ...
        await pool.query(query, function (err, rows) {
            if (err) {
                req.flash('error', err);
                res.render('admin/news/showNews/show-news', {data: ''});
            } else {
                res.render('admin/news/showNews/show-news', {data: rows})
            }
        })
    } else {
    ...
    if (user) {
        ...
        pool.query(query, function (err, rows, fields) {
            ...
            if (rows.length <= 0) {
                ...
                res.redirect('admin/news/showNews/show-news')
            } else { // if topic found
                // render to views/topic/edit.ejs template file
                res.render('admin/news/showNews/show-news', {
                    id: rows[0].id,
