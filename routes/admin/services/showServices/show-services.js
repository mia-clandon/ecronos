...
router.get('/', function(req, res, next) {
  ...
  if (user) {
    ...
    pool.query(query, function (err, rows) {
      if (err) {
        req.flash('error', err);
        res.render('admin/services/showServices/show-services', {data: ''});
      } else {
        res.render('admin/services/showServices/show-services', {data: rows})
      }
    })
    } else {
    ...
    if (user) {
      ...
      pool.query(query, function (err, rows, fields) {
        ...
        if (rows.length <= 0) {
          ...
          res.redirect('/admin/services/showServices/show-services')
        } else { // if topic found
          // render to views/topic/edit.ejs template file
          res.render('/admin/services/showServices/show-services', {
            id: rows[0].id,
